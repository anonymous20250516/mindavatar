...
def train(args):
    ...
    output_dim = train_dataset.cloth_detail_class_num
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for batch_idx, batch in progress_bar:
            ...
            cloth_detail_labels = batch['cloth_detail_idx']

            if not torch.is_tensor(cloth_detail_labels):
                cloth_detail_labels = torch.tensor(cloth_detail_labels, dtype=torch.long)
            
            cloth_detail_labels = cloth_detail_labels.to(device)

            optimizer.zero_grad()
            outputs = model(fmri)

            loss = criterion(outputs, cloth_detail_labels)
        ...
        with torch.no_grad():
            for batch in test_loader:
                ...
                all_cloth_detail_labels.append(cloth_detail_labels.detach().cpu())
        ...
        metrics = {}
        all_cloth_detail_labels = torch.cat(all_cloth_detail_labels, dim=0)
        metrics['cloth_detail_acc'], cloth_detail_acc_per_sample = compute_classification_metrics(all_outputs, all_cloth_detail_labels)
        ...
        if test_loss < best_test_loss:
            ...
            np.save(output_path, all_cloth_detail_labels.numpy())
