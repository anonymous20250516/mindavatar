...
def train(args):
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for batch_idx, batch in progress_bar:
            ...
            gender_labels = batch['texture_id']

            if not torch.is_tensor(gender_labels):
        ...
        with torch.no_grad():
            for batch in test_loader:
                ...
                all_gender_labels.append(gender_labels.detach().cpu())
        ...
        metrics['gender_acc'], gender_acc_per_sample = compute_classification_metrics(all_outputs, all_gender_labels)
        ...
        # 如果需要，可保存最佳模型checkpoint
        if test_loss < best_test_loss:
            best_test_loss = test_loss
            best_epoch = epoch
            ckpt_path = os.path.join(args.ckpt_dir, f"sub_{args.sub}_best.pth")
            os.makedirs(args.ckpt_dir, exist_ok=True)
            ...
            np.save(output_path, all_gender_labels.numpy())
